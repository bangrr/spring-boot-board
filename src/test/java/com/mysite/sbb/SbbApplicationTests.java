...
class SbbApplicationTests {
	...
	void findAllQuestion() {
		...
		Question q1 = all.get(0); // findAll 로 가져온 List 중에 첫 번째 데이터
		assertEquals("sbb가 무엇인가요?", q1.getSubject());

		Question q2 = all.get(1); // findAll 로 가져온 List 중에 첫 번째 데이터
	...
	void findQuestionById() {
		...
		} else { // 책에는 없던 내용인데 else 처리를 해주지 않으면 oq 가 Optional.EMPTY 일때도 테스트가 성공한다.
			fail("해당 id의 질문은 없습니다.");
		}
	}

	@Test
	void findQuestionBySubject() {
